    ...
    public Person getPerson(String uri){
    	...
    	try {
    		String query = "PREFIX ns: <http://rdf.freebase.com/ns/> " +
    				"SELECT DISTINCT ?actorName ?actorDes ?actor ?dateOfBirth ?placeOfBirth ?gender " +
    				"FROM <http://fmb.org> " +
    				"WHERE { " +
    				"?actor ns:type.object.type ns:film.actor. " +
    				"?actor ns:type.object.name ?actorName. " +
    				"?actor ns:common.topic.description ?actorDes. " +
    				"?actor ns:people.person.date_of_birth ?dateOfBirth. " + 
    				"?actor ns:people.person.place_of_birth/ns:type.object.name ?placeOfBirth. " +
    				"?actor ns:people.person.gender/ns:type.object.name ?gender. " + 
    				"FILTER(?actor = ns:%s)}";
    ...
    private Person createPerson(JSONObject actorJson, int getImagesCount){
    	...
    	if(actorJson.has("dateOfBirth")){
			try {
				...
				person.setBirthday(formatter.parse(birthString));
